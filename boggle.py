...
class Boggle():
    ...
    def read_dict(self, dict_path):
        ...
        # WHAT IS DICT_PATH? WHERE IS IT DEFINED?

        dict_file = open(dict_path)
        words = [w.strip() for w in dict_file]
        # strip strips the blank spaces before and after each dict entry
        dict_file.close()
    ...
    def make_board(self):
        ...
        for y in range(5):
            ...
            # after using import string up top, you can call this for the alphabet in a string, either up or lower case
            board.append(row)
    ...
    def check_valid_word(self, board, word):
        ...
        # read_dict RETURNS WORDS, NOT WORD - WHAT IS WORD? DO WE HAVE ACCESS TO WORDS THROUGH SELF?

        word_exists = word in self.words
        # does the word exist in the dictionary?
        valid_word = self.find(board, word.upper())
        # does the word exist on the board?
